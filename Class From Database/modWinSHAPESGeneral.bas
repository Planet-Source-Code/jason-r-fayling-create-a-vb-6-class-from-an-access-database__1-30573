...
Public Function GetUnusedFilename(ByVal sPath As String, ByVal sName As String, ByVal sExtension As String, Optional ByVal lStartAt = 1) As String

On Error GoTo ErrorCode

Dim sNewFile As String

    lStartAt = Abs(lStartAt)
        
    If Right(sPath, 1) = "\" Then sPath = Mid(sPath, 1, Len(sPath) - 1)
    If Len(Dir(sPath, vbDirectory)) = 0 Then GoTo ClearVariables
    
    If InStr(1, sExtension, ".", vbBinaryCompare) = "0" Then sExtension = "." & sExtension
    
    If InStr(1, sExtension, "/", vbBinaryCompare) <> 0 Then GoTo ClearVariables
    If InStr(1, sExtension, "\", vbBinaryCompare) <> 0 Then GoTo ClearVariables
    If InStr(1, sExtension, ":", vbBinaryCompare) <> 0 Then GoTo ClearVariables
    If InStr(1, sExtension, "*", vbBinaryCompare) <> 0 Then GoTo ClearVariables
    If InStr(1, sExtension, "?", vbBinaryCompare) <> 0 Then GoTo ClearVariables
    If InStr(1, sExtension, Chr(34), vbBinaryCompare) <> 0 Then GoTo ClearVariables
    If InStr(1, sExtension, "<", vbBinaryCompare) <> 0 Then GoTo ClearVariables
    If InStr(1, sExtension, ">", vbBinaryCompare) <> 0 Then GoTo ClearVariables
    If InStr(1, sExtension, "|", vbBinaryCompare) <> 0 Then GoTo ClearVariables
       
    If InStr(1, sName, "/", vbBinaryCompare) <> 0 Then GoTo ClearVariables
    If InStr(1, sName, "\", vbBinaryCompare) <> 0 Then GoTo ClearVariables
    If InStr(1, sName, ":", vbBinaryCompare) <> 0 Then GoTo ClearVariables
    If InStr(1, sName, "*", vbBinaryCompare) <> 0 Then GoTo ClearVariables
    If InStr(1, sName, "?", vbBinaryCompare) <> 0 Then GoTo ClearVariables
    If InStr(1, sName, Chr(34), vbBinaryCompare) <> 0 Then GoTo ClearVariables
    If InStr(1, sName, "<", vbBinaryCompare) <> 0 Then GoTo ClearVariables
    If InStr(1, sName, ">", vbBinaryCompare) <> 0 Then GoTo ClearVariables
    If InStr(1, sName, "|", vbBinaryCompare) <> 0 Then GoTo ClearVariables
        
    sNewFile = sPath & "\" & sName & sExtension
    If DoesFileExist(sNewFile) = False Then
        GetUnusedFilename = sNewFile
        Exit Function
    End If
    
    Do
        sNewFile = CStr(sPath & "\" & sName & lStartAt & sExtension)
        lStartAt = lStartAt + 1
    Loop Until DoesFileExist(sNewFile) = False
...
Public Function SetFormParent(ByVal lChildHwnd As Long, ByVal lParentHwnd As Long) As Long
    SetFormParent = SetWindowLong(lChildHwnd, GWL_HWNDPARENT, lParentHwnd)
End Function
